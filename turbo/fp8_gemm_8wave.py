...
def asm_mma_do(a, b, c, mode="2", cbsz=0, blgp=0):
    """fp8 16x16x128 MFMA via inline asm, pinning dst register class to avoid the
    v_accvgpr_read/write shuffle the mma_atom path emits. mode 2 (=a,v,v,0): AGPR in-place."""
...
class Mfma16x16x128:
    """asm_mode 0=mma_atom (accvgpr shuffle); 2=AGPR in-place asm; 3=VGPR in-place asm."""
...
def compile_fp8_gemm_8w(
    *,
    ...
    asm_mma: int = 2,  # 2=AGPR in-place inline-asm MFMA (kills accvgpr shuffle, default); 0=atom
):
    BLOCK_K = 128
    ...
    # N_ACCUMS vec4 x 4 AGPR = 16*N_ACCUMS, =128 for 256x256); without the hint the
    # allocator under-provisions AGPR and the in-place accumulators alias -> wrong result.
    ...
    def launch_gemm(
        ...
        _attrs = {"rocdl.waves_per_eu": waves_per_eu, "rocdl.flat_work_group_size": "512,512"}
        if _agpr_eff != 0:
