"""2000-sample benchmark to find true best dispatch time."""
...
above5500 = sum(1 for t in ts if tf(t) >= 5500)
print("2000-sample M%dN%dK%d: min=%.1f p1=%.1f p5=%.1f p10=%.1f med=%.1f TF | n>=5500: %d" % (
    M, N, K, tf(ts[0]), tf(ts[19]), tf(ts[99]), tf(ts[199]), tf(statistics.median(ts)), above5500))
