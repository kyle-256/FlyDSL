...
def compute(mode,a,b,sav,sbv):
    res_ty=Vec.make_type(4,fx.Float32); zero=Vec.filled(4,0.0,fx.Float32); z4=Vec.filled(4,0,fx.Int32)
    if mode=="intr":
        c=zero
        for _ in range_constexpr(N):
            c=rocdl.mfma_scale_f32_16x16x128_f8f6f4(res_ty,[pack_i32x4_i32x8(Vec(a),z4),pack_i32x4_i32x8(Vec(b),z4),c,4,4,0,sav,0,sbv])
        return Vec(c)
    SNOP=int(__import__("os").environ.get("SNOP","8"))
    one="v_mfma_scale_f32_16x16x128_f8f6f4 $0, $1, $2, $0, $3, $4 op_sel_hi:[0,0,0] cbsz:4 blgp:4"+("\ns_nop "+str(SNOP) if SNOP>=0 else "")
    asm="\n".join([one]*N)
    return Vec(_llvm.inline_asm(res_ty,[_raw(a),_raw(b),_raw(sav),_raw(sbv),_raw(zero)],asm,"=v,v,v,v,v,0",has_side_effects=True))
def build(mode):
    @flyc.kernel(name=f"asmblk_{mode}", known_block_size=[64,1,1])
    def k(out: fx.Tensor, av: fx.Tensor, bv: fx.Tensor, sav: fx.Int32, sbv: fx.Int32):
        ...
        outc=compute(mode, ld4(av), ld4(bv), sav, sbv)
        bo=buffer_ops.extract_base_index(out,address_space=1)
        base=_llvm.inttoptr(ir.Type.parse("!llvm.ptr<1>"),_raw(fx.Int64(bo)))
        for i in range_constexpr(4):
            ptr=buffer_ops.get_element_ptr(base,byte_offset=_raw((lane*fx.Int32(4)+fx.Int32(i))*fx.Int32(4)),elem_type=T.i8)
            _llvm.StoreOp(_raw(outc[i]),ptr)
    @flyc.jit
    def launch(out:fx.Tensor,av:fx.Tensor,bv:fx.Tensor,sav:fx.Int32,sbv:fx.Int32,stream:fx.Stream):
        k(out,av,bv,sav,sbv).launch(grid=(1,1,1),block=(64,1,1),stream=stream)
    return launch
def main():
    ...
    for mode in ("intr","asm"):
        out=torch.zeros(64*4,dtype=torch.float32,device=d)
        cc=flyc.compile(build(mode),out,av,bv,sa,sa,st);out.zero_();cc(out,av,bv,sa,sa,st);torch.cuda.synchronize();res[mode]=out.clone()
    diff=(res["intr"]-res["asm"]).abs().max().item()
    print(f"N={N} max|intr-asm|={diff:.3g}  match={diff<1e-2}  intr[:4]={res['intr'][:4].tolist()}")
main()
